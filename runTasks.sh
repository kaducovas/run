...
queuedTasks=$(expr $(PGPASSWORD=2019_constantedeplanck psql -h 201.17.19.173 -p 6432 -d ringerdb -U ringer -c "select count(*) from tasks where status = 'queued' and context = 'official';"  | sed -n 3p ))
maxtasks=$1
#queuedTasks=11
...
if [ $queuedTasks -gt 0 ]
 then
 expr $queuedTasks
 avail=$(expr $maxtasks - $(docker ps | grep saphyra:2.3 | wc -l))
 echo $avail
 if [ $avail -gt 0 ]
  then
